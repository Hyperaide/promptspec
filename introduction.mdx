...
PromptSpec is an open-source, language-agnostic framework designed to streamline and standardize the process of creating, deploying, and testing AI prompts across various AI models and platforms. It offers a structured approach to building prompts, enabling developers and researchers to create complex AI interactions with ease and consistency.
...
tests:
  - name: "Test with specific location"
    ...
    response_pattern: ".*\\b\\d{1,2}\\b.*"  # Pattern matching a numeric temperature value
  ...
  - name: "Test with vague weather question"
    ...
    response_pattern: ".*umbrella.*Seattle.*"
  ...
  - name: "Test without location, expecting a prompt for location"
    ...
    response_pattern: ".*location.*"  # Expecting a response that asks for the user's location
```
